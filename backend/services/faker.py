...
class FakeTweetService:
    """Service for generating fake airline tweets."""
    
    def __init__(self):
        self.airlines = ['Virgin America', 'United', 'Southwest', 'Delta', 'US Airways', 'American']
        self.sentiments = ['neutral', 'positive', 'negative']
    ...
    def generate_tweet(self) -> Tweet:
        ...
        if sentiment == 'negative':
            negativereason = random.choice(self.negative_reasons[1:]) 
        elif sentiment == 'neutral':
            negativereason = random.choice(self.negative_reasons) 

        handle = self.handles[airline]
    ...
    def generate_batch(self, batch_size: int = 10) -> List[Tweet]:
        """Generate a batch of fake tweets."""
